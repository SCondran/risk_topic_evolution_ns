...
class SOM(object):
    ...
    def updateMike(self,
                    ...
                    maxtrainlen=np.Inf,
                    theNewData=None):
                    #theNewData=self.data_raw):

            """
            This just updates the dataset the the the SOM points to. Everything else remains the same
            ...
            self._data = self._normalizer.normalize(theNewData) if self._normalizer else theNewData
            #self._data = normalizer.normalize(theNewData) if normalizer else theNewData
            self._dim = theNewData.shape[1]
            self._dlen = theNewData.shape[0]
            self.data_raw = theNewData
